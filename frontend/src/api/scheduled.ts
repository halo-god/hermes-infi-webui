...
export const scheduledApi = {
  ...
  async create(payload: { name: string; agent_id: string; profile_id?: string; prompt: string; cron: string; enabled?: boolean }): Promise<ScheduledTask> {
    return (await http.post<ScheduledTask>("/scheduled", payload)).data;
  },
  async update(id: string, payload: Partial<{ name: string; agent_id: string; profile_id?: string; prompt: string; cron: string; enabled: boolean }>): Promise<ScheduledTask> {
